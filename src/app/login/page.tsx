          ...
          <div className="flex gap-2 mb-7">
            {(["login", "register"] as const).map((v) => (
              <button key={v} onClick={() => { setView(v); setError(""); }}
                className={clsx("flex-1 py-2.5 rounded-xl text-sm font-medium transition-all duration-200", view === v ? "bg-gradient-to-r from-[#D4AF37] to-[#B8960F] text-black shadow-lg shadow-[#D4AF37]/20" : "bg-white/5 text-white/50 hover:text-white/70")}>
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleLogin()}
                  placeholder="faizi@digitalpointllc.com" className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-white placeholder:text-white/20 focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/40 text-sm" />
              </div>
              <div>
                ...
                <div className="relative">
                  <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleLogin()}
                    placeholder="Enter password" className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-white placeholder:text-white/20 focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/40 pr-12 text-sm" />
            ...
            <div className="space-y-4">
              <div>
                ...
                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Your name"
                  className="w-full bg-white/[0.04] border border-white/[0.08] rounded-xl px-4 py-3 text-white placeholder:text-white/20 focus:outline-none focus:ring-2 focus:ring-[#D4AF37]/40 text-sm" />
              </div>
              <div>
                <label className="block text-xs text-white/50 mb-2 font-medium uppercase tracking-wider">Email</label>
              ...
              {demoUsers.map((user, i) => (
                <button key={i} onClick={() => quickLogin(user)}
                  ...
                  <div className={clsx("w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold shrink-0",
                    i === 0 ? "bg-[#D4AF37]/20 text-[#D4AF37]" : i === 1 ? "bg-[#0EA5E9]/20 text-[#0EA5E9]" : i === 2 ? "bg-[#22C55E]/20 text-[#22C55E]" : "bg-white/10 text-white/50"
